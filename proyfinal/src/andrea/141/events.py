...
class Event:
    ...
    def __init__(self, group_id, event_id, task_id=None):
...
class TaskEvent(Event):
    def __init__(self, eid, tid):
        Event.__init__(self, 'task', eid, tid)

    def __str__(self):
        if self.stats:
            #      ti  tf  dur   rank     tid    res  solvt
            fmt = '%s--%s  (%s)   %3d   %-30s %8s   %s st %5d pv  %6d tv  %6d cl\n'
	    solver_time = ''
	    if 'solver.msecs' in self.stats:
            ...
            row = fmt % (
             ...
             self.src, self.tid, self.res, solver_time, int(pv), int(tv), int(cl))
        elif self.res == 'ERROR':
            #      ti  tf  dur   rank     tid    res
            fmt = '%s--%s  (%s)   %3d   %-30s %8s   %s20\n'
            row = fmt % (
             secs2human(self.t_i, msecs=False), secs2human(self.t_f, msecs=False), secs2human(self.dur),
             self.src, self.tid, self.res, str(self.etc))
        else:
            #      ti  tf  dur   rank     tid    res
            fmt = '%s--%s  (%s)   %3d   %-30s %8s\n'
            row = fmt % (
             secs2human(self.t_i, msecs=False), secs2human(self.t_f, msecs=False), secs2human(self.dur),
             self.src, self.tid, self.res)
        return row
