...
from tools    import Alloy, JForge, Reason
...
class Worker(Process):
    ...
    def init_tools(self):
        ...
        self.jforge   = make_tool(JForge, settings.jforge)
        self.tools    = dict({'als': self.alloy, 'jf': self.jforge})
	self.rcstrs   = dict({'als': Alloy.RC_STR, 'jf': JForge.RC_STR})
    ...
    def handle_mission_jf(self, tid, jf_file_contents):
        my_local_dir = settings.local_store_dir(self.my_rank)
        jf_path = path.join(my_local_dir, tid)
	write_file(jf_path, jf_file_contents)
        self.current_tool.launch(jf_path)
        self.tool_start_time = network.time()
