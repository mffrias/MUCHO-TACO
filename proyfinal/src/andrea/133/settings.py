...
def read(conf_path):
    ...
    global experiment, queues, logging, paths, alloy, jforge, jmlforge, jfplugin
    ...
    # hacky postprocessing of jforge additional classpaths (improve!)
    jforge_required_extra_cps = ['forge', 'kodkod', 'antlr',
      'junit', 'soot', 'jasmin', 'polyglot', 'commons_cli']
    for cp in ['cp_' + p for p in jforge_required_extra_cps]:
        jforge['class_path'] += pathsep + jforge[cp]
