...
public final class Reporter extends A4Reporter {
    ...
    public void parseAndTypeCheck(String filename) {
        ...
        if(TranslateAlloyToKodkod.boundsHackEnabled)
            reportDatum("Guigna bounds hack", "enabled");
        if(TranslateAlloyToKodkod.removeAlloySyntaxChecksHackEnabled)
	    reportDatum("Syntax checks hack", "enabled");
        if(kodkod.ast.Relation.enumerateBooleanQueriesHackEnabled)
            reportDatum("Enum bool queries hack", "enabled");        
    ...
    public void translate(String solver, int bitw, int maxseq, int skold, int symm) {
        ...
        else if(kodkod.engine.fol2sat.Translator.bedHackEnabled)
            reportTitle("Translating Kodkod to BED");
        else
    ...
    public void resultSAT(Object command, long solvingTime, Object solution) {
        ...
        rfm.alloy.CLI.solvingTime = solvingTime;
    }
    ...
    private void reportTitle(String phaseName)
      { long ms_elapsed = time();
        System.out.println();
        String s = String.format("%08d  %s", ms_elapsed, phaseName);
