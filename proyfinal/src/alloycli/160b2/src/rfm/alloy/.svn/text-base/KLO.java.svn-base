...
public class KLO {
    ...
    public static void main(String[] args) throws Err, IOException {
        ...
        log.parseAndTypeCheck("base.als");
        Module world = CompUtil.parseEverything_fromFile(log, null, "base.als");

        for(String filename: commandLine.getArgs()) {
            
            //log.parseAndTypeCheck(filename);

            world.addGlobal("factA",
                CompUtil.parseOneExpression_fromString(
                    world,  edu.mit.csail.sdg.alloy4.Util.readAll(filename+"A.als"))
                    );
            world.addGlobal("factB",
                CompUtil.parseOneExpression_fromString(
                    world,  edu.mit.csail.sdg.alloy4.Util.readAll(filename+"B.als"))
                    );
    ...
    public static A4Options xlateCommandLine(CommandLine commandLine) {
        ...
        if(commandLine.hasOption("s")) {
            ...
            else if(solver == "sat4j")
                alloyOptions.solver = A4Options.SatSolver.SAT4J;
            else if(solver == "zchaff")
                alloyOptions.solver = A4Options.SatSolver.ZChaffJNI;
        } else alloyOptions.solver = A4Options.SatSolver.MiniSatJNI;
    ...
    public static Options defineCommandLineOptions() {
        ...
        solv.addOption(new Option("s", "solver",  true,  "solve via minisat (default), zchaff or sat4j"));
    ...
    public static void showUsage(Options options) {
        ...
        System.out.println("");
        formatter.printHelp(79, "rfm.alloy.KLO [options] specfile.als\n", "", options,
    ...
    public static void showVersion() {
        ...
        System.out.println("   Command line interface version:   " + VERSION_KLO);
        System.out.println("");

    }

    public static final String VERSION_KLO = "1.5.6";
