...
public class Z {
	...
	for(CommandScope cs : command.scope) {
		System.out.println(cs.toString()); //(answer.eval(s));
	}

        answer = TranslateAlloyToKodkod.execute_command(log, world.getAllReachableSigs(), command, alloy4Options);

	//kodkod.instance.Bounds bds = answer.getBounds();
/*

	for(Sig s : world.getAllReachableSigs()) {
		System.out.println(answer.a2k(s)); //(answer.eval(s));
	}
*/
...
/*
        ...
        if(answer.satisfiable() && commandLine.hasOption("x")) {
        ...
        if(commandLine.hasOption("i"))
            ignoreSols = true;
                enumSolutions(log, maxSols, maxSecs, fileName, ignoreSols);
            } else {
...
*/
    ...
    public static void enumSolutions(Reporter log, int maxSols, int maxSecs, String outFile, boolean ignoreSols) throws Err, IOException {
    ...
    public static Options defineCommandLineOptions() {
        ...
        options.addOption("x", "xml", true, "save single/first solution to XML file");
    ...
    public static void showUsage(Options options) {
        ...
        formatter.setOptionComparator(new java.util.Comparator() {
            private final String appearanceOrder = "sbckentfixBvh";
    ...
    public static final String VERSION_CLI = "1.5.4";
